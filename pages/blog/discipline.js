import Layout from "../../components/Layout/Layout";
import { SITE_URL } from "../../consts/site";
import { Form } from "../../components/Form/Form";
import { SecondaryPreview } from "../../components/SecondaryPreview/SecondaryPreview";
import { VisibilityManager } from "../../components/shared/VisibilityManager";
import containerStyles from "../../styles/container.module.css";
import contentStyles from "../../styles/contentStyles.module.css";
import secondary_preview_27 from "../../images/secondary_preview_27.webp";
import { Button } from "../../components/shared/Button/Button";

const schemaOrg = {
  "@context": "https://schema.org",
  "@type": "BlogPosting",
"headline": "Внутренняя дисциплина: направленная медитация как путь к преодолению прокрастинации",
  "description": "Истинная дисциплина — результат внутренних психологических установок, а не просто сила воли. Методы направленной медитации позволяют эффективно выявлять и устранять глубинные подсознательные барьеры, вызывающие прокрастинацию, восстанавливать естественную продуктивность и мотивацию, формировать навык завершения начатых проектов. Персонализированный подход к каждому клиенту, быстрое достижение измеримых результатов, глубинная работа с подсознательными механизмами. Ключевые слова: внутренняя дисциплина, преодоление прокрастинации, восстановление продуктивности, направленная медитация мотивации, работа с подсознательными блоками, психология самодисциплины, профессиональная консультация, Казань, трансформация прокрастинации, восстановление мотивации, формирование самоконтроля, повышение личной эффективности, нейтрализация внутренних барьеров.",
  "image": "/_next/static/media/secondary_preview_23.webp",
  "url": `${SITE_URL}/blog/discipline`,
  "inLanguage": "ru",
  "datePublished": "2025-04-05",
  "publisher": {
    "@type": "Organization",
    "name": "INTEGRAAL",
    "url": `${SITE_URL}`,
    "logo": {
      "@type": "ImageObject",
      "url": "/_next/static/media/logo.ab1a421b.webp",
      "width": 378,
      "height": 400
    }
  },
  "author": {
    "@type": "Person",
    "name": "Александр Васильев",
    "url": `${SITE_URL}/#about`,
    "description": "Гипнотерапевт, специалист по работе с подсознанием и психосоматикой",
    "jobTitle": "Гипнотерапевт, специалист по работе с подсознанием"
  },
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": `${SITE_URL}/blog/discipline`
  },
  "keywords": "дисциплина, прокрастинация, продуктивность, направленная медитация, подсознание, психология",
  "about": [
    {
      "@type": "Thing",
      "name": "Психологические причины отсутствия дисциплины",
      "description": "Связь прокрастинации с подсознательными эмоциональными блоками"
    },
    {
      "@type": "Thing",
      "name": "Направленная медитация в развитии дисциплины",
      "description": "Методы работы с подсознанием для устранения блоков, мешающих быть дисциплинированным"
    }
  ],
  "offers": {
    "@type": "Offer",
    "price": "Бесплатная первая консультация",
    "priceCurrency": "RUB",
    "availability": "https://schema.org/InStock",
    "url": `${SITE_URL}/blog/discipline#message`,
    "description": "Запишитесь на консультацию, чтобы узнать истинную причину отсутствия дисциплины в вашей жизни."
  }
};

export default function () {


  const metaConfig = {
    title: "Почему не получается держать дисциплину — и как это изменить",
    description: "Анализ психологических причин отсутствия дисциплины и как направленная медитация помогает вернуть контроль над собой.",
    keywords: "дисциплина, прокрастинация, продуктивность, направленная медитация, подсознание, психология, консультация, Казань, работа с подсознанием, мотивация, самоконтроль",
    seoContent: "Дисциплина — это не про «силу воли», а про внутренние установки и подсознательные ограничения. Направленная медитация помогает обнаружить и убрать причины прокрастинации, вернуть продуктивность и мотивацию, научиться доводить дела до конца. Индивидуальный подход, быстрые изменения, глубокая работа с подсознанием. Ключевые слова: самодисциплина, откладывание дел, личная продуктивность, направленная медитация, подсознательные программы, психологическая поддержка, консультация, Казань, терапия подсознания, внутренняя мотивация, самоконтроль, эффективность.",
  };

  return (
    <Layout metaConfig={metaConfig} schemaOrg={schemaOrg}>
      <SecondaryPreview 
        text="Почему не получается держать дисциплину" 
        image={secondary_preview_27} 
      />
        <div className={containerStyles.container}>
          <VisibilityManager as="h1" className={contentStyles.title_middle}>
            Почему не получается держать дисциплину — и как это изменить
          </VisibilityManager>

          <VisibilityManager as="p">
            Вы точно знаете, что вам нужно делать. Ставите цели, пишете списки, начинаете и… снова срываетесь.
          </VisibilityManager>

          <VisibilityManager as="p">
            День проходит — и всё не так. Внутренне вы злитесь, обещаете себе «в следующий раз обязательно». Но это «обязательно» откладывается. Снова и снова.
          </VisibilityManager>

          <VisibilityManager as="p">
            Вы могли бы быть в другом месте. Зарабатывать больше, выглядеть лучше, действовать свободно. Но будто кто-то внутри ставит палки в колёса.  
            И это — не лень. И не слабая воля.
          </VisibilityManager>

          <div style={{ height: '1px' }} />
          <VisibilityManager as="h3" className={contentStyles.title_middle}>
            Что на самом деле мешает вам быть дисциплинированным
          </VisibilityManager>

          <VisibilityManager as="p">
            Дисциплина — это свобода делать то, что вы хотите. Но если внутри включается сопротивление — значит, есть причина.
          </VisibilityManager>

          <VisibilityManager as="p">
            Вот как это может звучать на уровне подсознания:
          </VisibilityManager>

          <VisibilityManager as="div">
            <p className={contentStyles.textDescription}>
              <span className={contentStyles.textSymbol}>●</span>
              {"  "}
              «Если я начну делать — мне станет тяжело» (ассоциация действия с давлением, с которым вы не справитесь);
            </p>
            <p className={contentStyles.textDescription}>
              <span className={contentStyles.textSymbol}>●</span>
              {"  "}
              «Если у меня получится — меня осудят, отвергнут, завидовать начнут» (память о детской травме, где успех был опасен);
            </p>
            <p className={contentStyles.textDescription}>
              <span className={contentStyles.textSymbol}>●</span>
              {"  "}
              «Если я стану другим — я перестану быть собой» (лояльность к семье, к прошлой версии себя, к боли);
            </p>
            <p className={contentStyles.textDescription}>
              <span className={contentStyles.textSymbol}>●</span>
              {"  "}
              «Я не верю, что смогу» (пережитый когда-то провал — и теперь мозг охраняет от повторения).
            </p>
          </VisibilityManager>

          <VisibilityManager as="p">
            Вы не осознаёте эти программы, но они вами управляют. И даже если вы внешне мотивированы — внутри вас может быть тот, кто шепчет: «Не надо».
          </VisibilityManager>

          <div style={{ height: '1px' }} />
          <VisibilityManager as="h3" className={contentStyles.title_middle}>
            Что происходит, если это не менять
          </VisibilityManager>

          <VisibilityManager as="p">
            Накапливается вина, усталость. Появляется ощущение «я не такой как надо».
          </VisibilityManager>

          <VisibilityManager as="p">
            С каждой недоделанной задачей растёт внутреннее давление. С каждой отсроченной целью — чувство, что время уходит, а вы топчетесь на месте.
          </VisibilityManager>

          <VisibilityManager as="p">
            Прокрастинация становится привычной, и вы начинаете верить, что «такой уж характер». Но это не правда. Это просто программа и от неё можно освободиться.
          </VisibilityManager>

          <div style={{ height: '1px' }} />
          <VisibilityManager as="h3" className={contentStyles.title_middle}>
            Как эта проблема решается через направленную медитацию
          </VisibilityManager>

          <VisibilityManager as="p">
            Направленная медитация — это метод, который позволяет дойти до корня проблемы и найти ту самую первую точку, где вы когда-то решили: «лучше не начинать».
          </VisibilityManager>

          <VisibilityManager as="p">
            Во время работы мы:
          </VisibilityManager>

          <VisibilityManager as="div">
            <p className={contentStyles.textDescription}>
              <span className={contentStyles.textSymbol}>●</span>
              {"  "}
              находим эпизод, где закрепилось сопротивление;
            </p>
            <p className={contentStyles.textDescription}>
              <span className={contentStyles.textSymbol}>●</span>
              {"  "}
              погружаемся в него через тело, ощущения, эмоции;
            </p>
            <p className={contentStyles.textDescription}>
              <span className={contentStyles.textSymbol}>●</span>
              {"  "}
              отпускаем, проживаем, переопределяем.
            </p>
          </VisibilityManager>

          <VisibilityManager as="p">
            В результате внутренний запрет уходит, и появляется то, что не достаётся ни на одном марафоне продуктивности: спокойная, устойчивая энергия на действие.
          </VisibilityManager>


          <div style={{ height: '1px' }} />
          <Button className={contentStyles.message} href={{ hash: "#message" }} status="Записаться" />

          <div style={{ height: '1px' }} />
          <VisibilityManager as="h3" className={contentStyles.title_middle}>
            Что меняется после сессии
          </VisibilityManager>

          <VisibilityManager as="div">
            <p className={contentStyles.textDescription}>
              <span className={contentStyles.textSymbol}>●</span>
              {"  "}
              Вы начинаете делать, не уговаривая себя;
            </p>
            <p className={contentStyles.textDescription}>
              <span className={contentStyles.textSymbol}>●</span>
              {"  "}
              Становится легко доводить до конца;
            </p>
            <p className={contentStyles.textDescription}>
              <span className={contentStyles.textSymbol}>●</span>
              {"  "}
              Исчезает ощущение внутреннего конфликта;
            </p>
            <p className={contentStyles.textDescription}>
              <span className={contentStyles.textSymbol}>●</span>
              {"  "}
              Возвращается вкус к жизни и энергия на реализацию;
            </p>
          </VisibilityManager>

          <VisibilityManager as="p">
            Если устали бороться — я помогу вам обрести над собой контроль, и дисциплина станет вашей второй натурой.
          </VisibilityManager>
          <p
            style={{
              fontSize: "12px",
              fontStyle: "italic",
            }}
          >
            Метод не является медицинским и не заменяет консультации
            специалиста
          </p>

        </div>
      
      <div id="message" />
      <Form
        type="Дисциплина (статья)"
      />
    </Layout>
  );
} 
